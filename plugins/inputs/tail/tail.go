...
import (
	...
	"github.com/defineiot/dpipe/data/parsers"
	"github.com/defineiot/dpipe/plugins/inputs"

	"github.com/defineiot/dpipe/internal/globpath"
)
...
func (t *Tail) Gather(acc data.Accumulator) error {
	return nil
}

func (t *Tail) Start(acc data.Accumulator) error {
	t.Lock()
	defer t.Unlock()
...
func init() {
	inputs.Add("tail", func() inputs.Input {
		return NewTail()
	})
}
