...
func Parse(buf []byte) ([]metrics.Metric, error) {
	return ParseWithDefaultTimePrecision(buf, time.Now(), "")
}

func ParseWithDefaultTime(buf []byte, t time.Time) ([]metrics.Metric, error) {
...
func ParseWithDefaultTimePrecision(
	...
	if len(buf) <= 6 {
		return []metrics.Metric{}, makeError("buffer too short", buf, 0)
	}
	metrics := make([]metrics.Metric, 0, bytes.Count(buf, []byte("\n"))+1)
	var errStr string
	i := 0
	for {
