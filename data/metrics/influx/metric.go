...
func New(
	...
	var thisType metrics.ValueType
	if len(mType) > 0 {
		thisType = mType[0]
	} else {
		thisType = metrics.Untyped
	}
...
func (m *metric) Type() metrics.ValueType {
	return m.mType
}
...
func (m *metric) Split(maxSize int) []metrics.Metric {
	if m.Len() <= maxSize {
...
func (m *metric) Copy() metrics.Metric {
	return copyWith(m.name, m.tags, m.fields, m.t)
}

func copyWith(name, tags, fields, t []byte) metrics.Metric {
	out := metric{
