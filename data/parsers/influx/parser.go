...
func (p *InfluxParser) ParseWithDefaultTimePrecision(buf []byte, t time.Time, precision string) ([]metrics.Metric, error) {
	if !bytes.HasSuffix(buf, []byte("\n")) {
		buf = append(buf, '\n')
	}
	// parse even if the buffer begins with a newline
	buf = bytes.TrimPrefix(buf, []byte("\n"))
	metrics, err := influx.ParseWithDefaultTimePrecision(buf, t, precision)
...
// successfully.
func (p *InfluxParser) Parse(buf []byte) ([]metrics.Metric, error) {
	return p.ParseWithDefaultTimePrecision(buf, time.Now(), "")
}

func (p *InfluxParser) ParseLine(line string) (metrics.Metric, error) {
