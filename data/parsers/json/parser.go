...
func (p *JSONParser) parseArray(buf []byte) ([]metrics.Metric, error) {
...
func (p *JSONParser) parseObject(metrics []metrics.Metric, jsonOut map[string]interface{}) ([]metrics.Metric, error) {
	...
	metric, err := influx.New(p.MetricName, tags, f.Fields, time.Now().UTC())
...
func (p *JSONParser) Parse(buf []byte) ([]metrics.Metric, error) {
...
func (p *JSONParser) ParseLine(line string) (metrics.Metric, error) {
